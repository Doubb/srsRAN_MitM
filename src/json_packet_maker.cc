...
*/
int jsonPacketMaker::json_to_packet(const std::string buf, uint8_t *dst_msg, int *size)
{
          ...
          for (Value::ConstMemberIterator c1 = obj3.MemberBegin(); c1 != obj3.MemberEnd(); ++c1)
          {
            ...
            for (Value::ConstMemberIterator msgType = obj4.MemberBegin(); msgType != obj4.MemberEnd(); ++msgType)
            {
              ...
              for (Value::ConstMemberIterator contents = obj5.MemberBegin(); contents != obj5.MemberEnd(); ++contents)
              {
                ...
                if (strcmp(rrcSetupRequest, contents->name.GetString()) == 0)
                { // If RRC Setup Request
                  ...
                  handle_rrc_setup_request(dst_msg, ue_id_type, ue_id_value, establishment_cause, spare, size);
                }

                else if (strcmp(rrcResumeRequest, contents->name.GetString()) == 0)
                { // If RRC Resume Request
                  const Value &obj6 = contents->value;
                  for (Value::ConstMemberIterator content = obj6.MemberBegin(); content != obj6.MemberEnd(); ++content)
                  {
                    //std::cout << content->name.GetString() << ": ";
                  ...
                  handle_rrc_resume_request(dst_msg, resume_identity, resume_mac_i, resume_cause, spare, size);
                }

                else if (strcmp(rrcReestablishmentRequest, contents->name.GetString()) == 0)
                { // If RRC Reestablishment Request
                  const Value &obj6 = contents->value;
                  for (Value::ConstMemberIterator content = obj6.MemberBegin(); content != obj6.MemberEnd(); ++content)
                  {
                    //std::cout << content->name.GetString() << ": ";
                  ...
                  handle_rrc_reestablishment_request(dst_msg, c_rnti, pci, short_mac_i, reest_cause, spare, size);
                }
                ...
                else
                {
                  ...
                  for (Value::ConstMemberIterator content = obj6.MemberBegin(); content != obj6.MemberEnd(); ++content)
                  {
                    ...
                    if (strcmp(rrcSecurityModeComplete, content->name.GetString()) == 0)
                    { // If RRC Security Mode Complete
                      handle_rrc_security_mode_complete(dst_msg, rrcTransactionIdentifier, size);
                    }

                    else if (strcmp(rrcResumeComplete, content->name.GetString()) == 0)
                    { // If RRC Resume Complete
                      handle_rrc_resume_complete(dst_msg, rrcTransactionIdentifier, size);
                    }

                    else if (strcmp(rrcSecurityModeFailure, content->name.GetString()) == 0)
                    { // If RRC Security Mode Failure
                      handle_rrc_security_mode_failure(dst_msg, rrcTransactionIdentifier, size);
                    }

                    else if (strcmp(rrcReconfigurationComplete, content->name.GetString()) == 0)
                    { // If RRC Reconfiguration Complete
                      handle_rrc_reconfiguration_complete(dst_msg, rrcTransactionIdentifier, size);
                    }

                    else if (strcmp(rrcRelease, content->name.GetString()) == 0)
                    { // If RRC Release
                      handle_rrc_release(dst_msg, rrcTransactionIdentifier, size);
                    }

                    else if (strcmp(rrcSecurityModeCommand, content->name.GetString()) == 0)
                    { // If RRC Security Mode Command
                      const Value &obj7 = content->value;
                      ...
                      for (Value::ConstMemberIterator smc = obj7.MemberBegin(); smc != obj7.MemberEnd(); ++smc)
                      {
                          ...
                          for (Value::ConstMemberIterator algorithms = obj8.MemberBegin(); algorithms != obj8.MemberEnd(); ++algorithms)
                          {
                            ...
                            for (Value::ConstMemberIterator algorithm = obj9.MemberBegin(); algorithm != obj9.MemberEnd(); ++algorithm)
                            {
                              ...
                              if (strcmp(ciphering_algorithm, algorithm->name.GetString()) != 0)
                              {
                                ...
                                {
                                  handle_rrc_security_mode_command(dst_msg, rrcTransactionIdentifier, cipheringAlgorithm, integrityAlgorithm, false, "n", size);
                                }
                              }
                        ...
                        else if (strcmp(lateNonCriticalExtension, smc->name.GetString()) == 0)
                        {
                          ...
                          if (smc + 1 == obj7.MemberEnd())
                          {
                            handle_rrc_security_mode_command(dst_msg, rrcTransactionIdentifier, cipheringAlgorithm, integrityAlgorithm, false, lateNonCritExts, size);
                          }
                        }
                        else if (strcmp(nonCriticalExtension, smc->name.GetString()) == 0)
                        {
                          ...
                          if (smc + 1 == obj7.MemberEnd())
                          {
                            handle_rrc_security_mode_command(dst_msg, rrcTransactionIdentifier, cipheringAlgorithm, integrityAlgorithm, true, lateNonCritExts, size);
                          }
                        }
                      }
                    }

                    else if (strcmp(rrcReject, content->name.GetString()) == 0)
                    { // If RRC Reject
                      //std::cout << buf << std::endl;
                      const Value &obj7 = content->value;
                      ...
                      for (Value::ConstMemberIterator time = obj7.MemberBegin(); time != obj7.MemberEnd(); ++time)
                      {
                        ...
                        handle_rrc_reject(dst_msg, waitTime, size);
                      }
                    }

                    else if (strcmp(rrcUeCapEnquiry, content->name.GetString()) == 0)
                    { // If RRC UE Capability Enquiry
                      //std::cout << buf << std::endl;
                      const Value &obj7 = content->value;
                        ...
                        for (Value::ConstValueIterator reqList = obj8.Begin(); reqList != obj8.End(); ++reqList)
                        {
                          ...
                          for (Value::ConstMemberIterator req = obj9.MemberBegin(); req != obj9.MemberEnd(); ++req)
                          {
                            ...
                            if (strcmp(rat_type, req->name.GetString()) != 0)
                            {
                              ...
                              handle_rrc_ue_cap_enquiry(dst_msg, rrcTransactionIdentifier, ratType, capReqFilter, size);
                            }
                            else
                            {
                    ...
                    else if (strcmp(rrcUeCapInformation, content->name.GetString()) == 0)
                    { // If RRC UE Capability Information
                      const Value &obj7 = content->value;
                        ...
                        for (Value::ConstValueIterator containerList = obj8.Begin(); containerList != obj8.End(); ++containerList)
                        {
                          ...
                          for (Value::ConstMemberIterator container = obj9.MemberBegin(); container != obj9.MemberEnd(); ++container)
                          {
                            ...
                            if (strcmp(rat_type, container->name.GetString()) != 0)
                            {
                              ...
                              handle_rrc_ue_cap_information(dst_msg, rrcTransactionIdentifier, ratType, capRatContainer, size);
                            }
                            else
                            {
                    ...
                    else if (strcmp(rrcReconfiguration, content->name.GetString()) == 0)
                    { // If RRC Reconfiguration
                      const Value &obj_ = content->value;
                      ++itrr;
                      const Value &obj__ = *itrr;
                      handle_rrc_reconfiguration(dst_msg, rrcTransactionIdentifier, size, obj_, obj__);
                      isRrcReconfiguration = 1;
                    }

                    else if (strcmp(rrcReestablishment, content->name.GetString()) == 0)
                    { // If RRC Reestablishment
                      const Value &obj7 = content->value;
                      ...
                      handle_rrc_reestablishment(dst_msg, rrcTransactionIdentifier, next_hop, size);
                    }

                    else if (strcmp(rrcResume, content->name.GetString()) == 0)
                    { // If RRC Resume
                      const Value &obj_ = content->value;
                      ++itrr;
                      const Value &obj__ = *itrr;
                      handle_rrc_resume(dst_msg, rrcTransactionIdentifier, size, obj_);
                      isRrcResume = 1;
                    }
                    ...
                    else if (strcmp(dlInfoTransfer, content->name.GetString()) == 0 || strcmp(ulInfoTransfer, content->name.GetString()) == 0)
                    {
                      ...
                      if (strcmp(nasAuthenticationRequest, nas_message_type.c_str()) == 0)
                      { // If NAS Authentication Request
                        handle_nas_authentication_request(dst_msg, rrcTransactionIdentifier, dedicatedNAS, size, obj_);
                        isNasAuthenticationRequest = 1;
                      }

                      else if (strcmp(nasAuthenticationResponse, nas_message_type.c_str()) == 0)
                      { // If NAS Authentication Response
                        isNasAuthenticationResponse = 1;

                        handle_nas_authentication_response(dst_msg, dedicatedNAS, size, obj_);
                      }

                      else if (strcmp(nasAuthenticationReject, nas_message_type.c_str()) == 0)
                      { // If NAS Authentication Reject
                        isNasAuthenticationReject = 1;

                        handle_nas_authentication_reject(dst_msg, size);
                      }

                      else if (strcmp(nasAuthenticationFailure, nas_message_type.c_str()) == 0)
                      { // If NAS Authentication Failure
                        isNasAuthenticationFailure = 1;

                        handle_nas_authentication_failure(dst_msg, size);
                      }

                      else if (strcmp(nasSecurityModeCommand, nas_message_type.c_str()) == 0)
                      { // If NAS Security Mode Command
                        handle_nas_security_mode_command(dst_msg, rrcTransactionIdentifier, dedicatedNAS, size, obj_);
                        isNasSecurityModeCommand = 1;
                      }

                      else if (strcmp(nasSecurityModeReject, nas_message_type.c_str()) == 0)
                      { // If NAS Security Mode Reject
                        handle_nas_security_mode_reject(dst_msg, size);
                        isNasSecurityModeReject = 1;
                      }

                      else if (strcmp(nasRegistrationReject, nas_message_type.c_str()) == 0)
                      { // If NAS Registration Reject
                        handle_nas_registration_reject(dst_msg, size);
                        isNasRegistrationReject = 1;
                      }
                    }

                    else if (strcmp(rrcSetup, content->name.GetString()) == 0)
                    { // If RRC Setup
                      const Value &obj7 = content->value;
                      ...
                      handle_rrc_setup(dst_msg, rrcTransactionIdentifier, srb_identity, masterCellGroupContent, reestablish_pdcp_present, discard_on_pdcp_present, size);
                    }

                    else if (strcmp(rrcSetupComplete, content->name.GetString()) == 0)
                    { // If RRC Setup Complete (RRC + NAS)
                      const Value &obj7 = content->value;
                      ...
                      ++itrr;
                      const Value &obj_ = *itrr;
                      handle_rrc_setup_complete(dst_msg, rrcTransactionIdentifier, plmnIdentity, dedicatedNAS, size, obj_);
                      isSetupComplete = 1;
                    }
                  }
                }
...
void jsonPacketMaker::handle_rrc_setup_request(uint8_t *dst_msg, std::string ue_id_type, std::string ue_id_value, std::string establishment_cause, std::string spare, int *size)
{
  ...
  memcpy(dst_msg, pdu->msg, pdu->N_bytes);
  *size = pdu->N_bytes;

  // Testing...
}

void jsonPacketMaker::handle_rrc_setup(uint8_t *dst_msg, int rrcTransactionIdentifier, int srb_identity, std::string masterCellGroup, bool reestablish_pdcp_present, bool discard_on_pdcp_present, int *size)
{
...
void jsonPacketMaker::handle_rrc_resume(uint8_t *dst_msg, int rrcTransactionIdentifier, int *size, const rapidjson::Value &obj)
{
...
void jsonPacketMaker::handle_rrc_resume_request(uint8_t *dst_msg, std::string resume_identity, std::string resume_mac_i, std::string resume_cause, std::string spare, int *size)
{
...
void jsonPacketMaker::handle_rrc_resume_complete(uint8_t *dst_msg, int rrcTransactionIdentifier, int *size)
{
...
void jsonPacketMaker::handle_rrc_security_mode_complete(uint8_t *dst_msg, int rrcTransactionIdentifier, int *size)
{
...
void jsonPacketMaker::handle_rrc_security_mode_failure(uint8_t *dst_msg, int rrcTransactionIdentifier, int *size)
{
...
void jsonPacketMaker::handle_rrc_security_mode_command(uint8_t *dst_msg, int rrcTransactionIdentifier, std::string cipheringAlgorithm, std::string integrityAlgorithm, bool non_crit_ext_present, std::string late_non_crit_ext, int *size)
{
...
void jsonPacketMaker::handle_rrc_reconfiguration(uint8_t *dst_msg, int rrcTransactionIdentifier, int *size, const rapidjson::Value &obj, const rapidjson::Value &obj2)
{
...
void jsonPacketMaker::handle_rrc_reconfiguration_complete(uint8_t *dst_msg, int rrcTransactionIdentifier, int *size)
{
...
void jsonPacketMaker::handle_rrc_reestablishment_request(uint8_t *dst_msg, int c_rnti, int pci, std::string short_mac_i, std::string reest_cause, std::string spare, int *size)
{
...
void jsonPacketMaker::handle_rrc_reestablishment(uint8_t *dst_msg, int rrcTransactionIdentifier, int next_hop, int *size)
{
...
void jsonPacketMaker::handle_rrc_reject(uint8_t *dst_msg, uint8_t waitTime, int *size)
{
...
void jsonPacketMaker::handle_rrc_release(uint8_t *dst_msg, int rrcTransactionIdentifier, int *size)
{
...
void jsonPacketMaker::handle_rrc_ue_cap_enquiry(uint8_t *dst_msg, int rrcTransactionIdentifier, std::string ratType, std::string capReqFilter, int *size)
{
...
void jsonPacketMaker::handle_rrc_ue_cap_information(uint8_t *dst_msg, int rrcTransactionIdentifier, std::string ratType, std::string capRatContainer, int *size)
{
...
void jsonPacketMaker::handle_rrc_setup_complete(uint8_t *dst_msg, int rrcTransactionIdentifier, int plmnIdentity, std::string dedicatedNAS, int *size, const Value &obj)
{
...
//            << std::endl;
  ...
  if (strcmp(nas_key_set_identifier.c_str(), "no key is available or reserved") == 0)
  {
  ...
  {
    reg_req.ng_ksi.nas_key_set_identifier = static_cast<srsran::nas_5g::key_set_identifier_t::nas_key_set_identifier_type_::options>(std::stoi(nas_key_set_identifier));
  }
...
void jsonPacketMaker::handle_nas_registration_reject(uint8_t *dst_msg, int *size)
{
  ...
  memcpy(dst_msg, pdu2->msg, pdu2->N_bytes);
  *size = pdu->N_bytes;
}

void jsonPacketMaker::handle_nas_authentication_request(uint8_t *dst_msg, int rrcTransactionIdentifier, std::string dedicatedNAS, int *size, const rapidjson::Value &obj)
{
  ...
  {
    auth_req.ng_ksi.nas_key_set_identifier = static_cast<srsran::nas_5g::key_set_identifier_t::nas_key_set_identifier_type_::options>(std::stoi(nas_key_set_identifier));
  }
...
void jsonPacketMaker::handle_nas_authentication_response(uint8_t *dst_msg, std::string dedicatedNas, int *size, const rapidjson::Value &obj)
{
  ...
  if (nas_pdu == nullptr)
  {
    std::cout << "nas_pdu creation failed" << std::endl;
  }
  ...
  ul_info_transfer->ded_nas_msg.resize(nas_pdu->N_bytes);
  memcpy(ul_info_transfer->ded_nas_msg.data(), nas_pdu->msg, nas_pdu->N_bytes);
...
void jsonPacketMaker::handle_nas_authentication_reject(uint8_t *dst_msg, int *size)
{
...
void jsonPacketMaker::handle_nas_authentication_failure(uint8_t *dst_msg, int *size)
{
...
void jsonPacketMaker::handle_nas_security_mode_command(uint8_t *dst_msg, int rrcTransactionIdentifier, std::string dedicatedNAS, int *size, const rapidjson::Value &obj)
{
  ...
  {
    sm_cmd.ng_ksi.nas_key_set_identifier = static_cast<srsran::nas_5g::key_set_identifier_t::nas_key_set_identifier_type_::options>(std::stoi(nas_key_set_identifier));
  }
...
void jsonPacketMaker::handle_nas_security_mode_reject(uint8_t *dst_msg, int *size)
{
